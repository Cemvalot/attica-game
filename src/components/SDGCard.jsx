  ...
  return (
    <div ref={refCallback} className="h-full w-full">
      <Tag
        ...
        className={cn(
          'relative flex h-full w-full flex-col items-center rounded-2xl border-4 text-center shadow-lg transition-transform duration-150',
          !readonly && !status && 'active:scale-95',
          status === 'wrong' && 'sdg-shake',
          status === 'correct' && 'sdg-pop',
          fillCell
      ...
      >
        ...
        >
          <SdgIcon
            sdgId={sdgId}
            alt={sdg.title}
            className={fillCell ? 'max-h-full max-w-full' : undefined}
          />
        </div>
        {showLabel && (
          <span
